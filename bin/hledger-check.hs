...
fixupJournal opts j = do
    ...
    let j' = (if cleared   opts then H.filterJournalTransactions (H.Status (Just H.Cleared))   else id)
           . (if pending   opts then H.filterJournalTransactions (H.Status (Just H.Pending))   else id)
           . (if uncleared opts then H.filterJournalTransactions (H.Not (H.Status (Just H.Cleared))) else id)
