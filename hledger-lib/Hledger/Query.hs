...
data Query = Any              -- ^ always match
           ...
           | Status (Maybe ClearedStatus) -- ^ match txns/postings with this cleared status
...
tests_parseQueryTerm = [
  "parseQueryTerm" ~: do
    ...
    "status:1" `gives` (Left $ Status (Just Cleared))
    "status:*" `gives` (Left $ Status (Just Cleared))
    "status:!" `gives` (Left $ Status (Just Pending))
    "status:0" `gives` (Left $ Not (Status (Just Cleared)))
    "status:" `gives` (Left $ Status Nothing)
...
parseStatus s | s `elem` ["*","1"] = Right $ Status (Just Cleared)
              | s `elem` ["!"]     = Right $ Status (Just Pending)
              | s `elem` ["0"]     = Right $ Not $ Status (Just Cleared)
              | s `elem` [""]      = Right $ Status Nothing
...
tests_filterQuery = [
 "filterQuery" ~: do
  ...
  (And [And [Status (Just Cleared),Depth 1]], not . queryIsDepth) `gives` Status (Just Cleared)
...
matchesPosting (Status (Just Cleared)) p = postingStatus p == Just Cleared
matchesPosting (Status (Just Pending)) p = postingStatus p == Just Pending
matchesPosting (Status Nothing) p = postingStatus p == Nothing
...
tests_matchesPosting = [
   "matchesPosting" ~: do
    ...
    assertBool "positive match on cleared posting status" $
                   (Status (Just Cleared))  `matchesPosting` nullposting{pstatus=Just Cleared}
    assertBool "negative match on cleared posting status" $
               not $ (Not $ Status (Just Cleared))  `matchesPosting` nullposting{pstatus=Just Cleared}
    assertBool "not-pending matches cleared" $
                   (Not $ Status (Just Pending)) `matchesPosting` nullposting{pstatus=Just Cleared}
    assertBool "not-pending matches no status" $
                   (Not $ (Status (Just Pending))) `matchesPosting` nullposting{pstatus=Nothing}
    assertBool "not-pending does not match pending" $
               not $ (Not $ Status (Just Pending)) `matchesPosting` nullposting{pstatus=Just Pending}
    assertBool "positive match on uncleared posting" $
                   (Not $ Status (Just Cleared)) `matchesPosting` nullposting{pstatus=Nothing}
    ...
    assertBool "positive match on true posting status acquired from transaction" $
                   (Status (Just Cleared)) `matchesPosting` nullposting{pstatus=Nothing,ptransaction=Just nulltransaction{tstatus=Just Cleared}}
...
matchesTransaction (Status (Just Cleared)) t = tstatus t == Just Cleared
matchesTransaction (Status (Just Pending)) t = tstatus t == Just Pending
matchesTransaction (Status Nothing) t = tstatus t == Nothing
