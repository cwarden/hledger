...
toggleCleared ui@UIState{aopts=uopts@UIOpts{cliopts_=copts@CliOpts{reportopts_=ropts}}} =
  ...
  where
    toggleCleared ropts@ReportOpts{clearedstatus_=Just (Status (Just Cleared))} = ropts{clearedstatus_=Nothing}
    toggleCleared ropts = ropts{clearedstatus_=Just $ Status (Just Cleared)}
...
togglePending ui@UIState{aopts=uopts@UIOpts{cliopts_=copts@CliOpts{reportopts_=ropts}}} =
  ...
  where
    togglePending ropts@ReportOpts{clearedstatus_=Just (Status (Just Pending))} = ropts{clearedstatus_=Nothing}
    togglePending ropts = ropts{clearedstatus_=Just $ Status (Just Pending)}
...
toggleUncleared ui@UIState{aopts=uopts@UIOpts{cliopts_=copts@CliOpts{reportopts_=ropts}}} =
  ...
  where
    toggleUncleared ropts@ReportOpts{clearedstatus_=Just (Not (Status (Just Cleared)))} = ropts{clearedstatus_=Nothing}
    toggleUncleared ropts = ropts{clearedstatus_=Just $ Not $ Status (Just Cleared)}
